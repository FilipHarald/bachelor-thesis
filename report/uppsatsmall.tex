...
In previous work within the area of OSS and requirement elicitation there are two highly relevant studies for this thesis. The first article~\cite{Mockus2002a} presents two case studies of the development and maintenance of major OSS projects: the Apache server and Mozilla. Where Mockus et al. address key questions about the development process, and about the software that is the result of the processes. Based on results from a earlier study made on Apache, covered in the article A Case Study of Open Source software Development: the Apache server, they framed a number of hypotheses that they conjectured would be true generally of open source developments. The second case study in the article~\cite{Mockus2002a} examined data from the Mozilla project based on the analyses and hypothesis that were framed from the Apache project. Mockus et al. came to the conclusion that the essential differences in which elements of commercial and open source processes coordinate, select, and assign work could be combined.

In the second highly relevant article,~\cite{Noll}, Noll et al. presents a case study of the OpenEMR, an open source project developing electronic medical record (EMR) software. The goal of the study was to understand how requirements are elicited, documented, agreed and validated in a small open source software project. The study followed the same approach as of an earlier study of the Firefox web browser~\cite{Noll2008a}. The comparison between the both projects showed that, similar to the Firefox study, the majority of OpenEMR requirements are asserted by developers. Documentation was informal, consisting mainly of archived discussions. Contributors to the OpenEMR project were medical practitioners such as doctors or clinic administrations, who use the product in their practices and also developers which does not use the product at all. The implication for software development in general is that developers can be a significant source of innovation.

%Communicate to the reader that what WE are doing has not been done just yet.

The articles together summarise the differences between open-source projects in different aspects. Mockus et al. investigate two cases from which they try to form theories on what it is that defines OSS-projects. They conclude the study by comparing their results with commercial, proprietary, projects~\cite{Mockus2002a}. Noll et al. on the other hand investigate the differences in requirement elicitation between large and small OSS-projects~\cite{Noll}. These articles are the two most relevant studies in comparing requirement elicitation in OSS projects.

%Shortly describe structure of report
The remainder of this paper is organised as follows: the next sub section will present the research questions; the next section will present our systematic literature review which we conducted to familiarise  with the subject; the next section will present the chosen method for this study; next are the results; they are followed by an analysis and discussion of their implications; lastly we present a conclusion and suggestions for future work.
...
\begin{table}
	\resizebox{!}{.35\paperheight}{\begin{tabular}{  p{0.6\textwidth}  | p{0.15\textwidth} | p{0.1\textwidth} | p{0.4\textwidth}}
		...
		A Case Study of Open Source software Development: the Apache server	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Agila projektledningsmetoder och motivation & \texttt{GOOGLE}	& 0 & SEARCHPHRASE \\\hline
		An analysis of requirements evolution in open source projects: recommendations for issue trackers	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Balancing act: community and local requirements in an open source development process	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Case Study Research: Design and Methods	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Challenges and recommendations for the design and conduct of global software engineering courses: A systematic review	&  \texttt{ACM}	& 0 & SEARCHPHRASE \\\hline
		Classifying Developers into Core and Peripheral: An Empirical Study on Count and Network Metrics	& Personal recommendation 	& 0 & SEARCHPHRASE \\\hline
		Comparative Case Studies	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Evolution in open source software: a case study	& \texttt{ACM}	& 0 & SEARCHPHRASE \\\hline
		Exploring the Structure of Complex Software Designs: An Empirical Study of Open Source and Proprietary Code	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Free/Libre open-source software development	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		From a Firm-Based to a Community-Based Model of Knowledge Creation: The Case of the Linux Kernel Development	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		How Can Open Source Software Development Help Requirements Management Gain the Potential of Open Innovation: An Exploratory Study	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		How Google Works	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Inflow and Retention in OSS Communities with Commercial Involvement	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Information systems success in free and open source software development: Theory and measures	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Innovation Model : Issues for Organization Science Open Source Software and the ''Private-Collective'' Innovation Model : Issues for Organization Science	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Open Sources: Voices from the Open Source Revolution	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Outsourcing to an Unknown Workforce: Exploring Opensourcing as a Global Sourcing Strategy	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Peripheral Developer Participation in Open Source Projects	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Requirements acquisition in open source development: Firefox 2.0	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Requirements elicitation in open source software development: a case study	& Course literature from previous course	& 0 & SEARCHPHRASE \\\hline
		Researching information systems and computing	& \texttt{ACM} or \texttt{IEEE}	& 0 & SEARCHPHRASE \\\hline
		Software Psychology: The Need for an Interdisciplinary Program	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		The agile manifesto	& \texttt{GOOGLE}	& 0 & SEARCHPHRASE \\\hline
		Two case studies of open source software development: Apache and Mozilla	& Snowballed	& 0 & SEARCHPHRASE \\\hline
		Why do users contribute to firm-hosted user communities? The case of computer-controlled music instruments	& Personal recommendation & 0 & SEARCHPHRASE \\\hline
...
The purpose of this study is to compare two different types of governing bodies for OSS-projects, firm driven and community driven. And with these results try to identify where companies can benefit from engaging in OSS development. For OSS projects in general all information regarding development process is public, this includes both documentation and code. The communication between project participants also tends to be public. This public access to data creates an opportunity for researchers to study the development process over time. With all this detailed information of a project's development process we have chosen to conduct a comparative case study on two OSS-projects, one of each type of governing bodies.
...
% apache and mozilla
As previously mentioned (section~\ref{related_work}) there have been two studies conducted that are similar to ours. In the first,~\cite{Mockus2002a}, Mockus et al. conduct an exploratory case study with the purpose of mapping how OSS-projects are conducted to form hypothesises on how they generally will perform. The authors compares their results with commercial projects. To make their generalisation trustworthy they have selected two cases which can be considered typical instances for an OSS-project, Apache and Mozilla. The authors present 6 research questions for which they motivate should be sufficient to compare with other projects~\cite{Mockus2002a}. In our case study we will use parts of the presented questions. However, because the second paper focuses on requirement elicitation, the majority of our data collection schema will be more similar to this that paper. The 6 questions are presented in Table~\ref{tab:apache}.
...
In this section we will discuss our findings in previous section \ref{detaileddata}, their implications and their relevance to our first two research questions. Then we will discuss, based on the first two research questions, the answer for our third research question. We consider the answers to the first two questions to be conclusive, and the answer for our third research question to be speculative.

\subsection{Conclusive}
\label{subsection:discussion_conclusive}
%Answer Research question 1
The research questions emph{What are the characteristics of community-driven OSS development?} and \emph{What are the characteristics of firm-driven OSS development?} will be discussed in this section. A summary of the differencies between the both cases will be presented up and further more a conclusion will be framed for each research question based on the result data in section \ref{detaileddata}.
...
When comparing the results from this thesis to the article ~\cite{Noll} the results shows that the majority of features are asserted by developers, based on either their personal experience, or knowledge of users? needs. Therefore we can conclude that the results from this thesis are general for several areas within the area of OSS development. Furthermore we also compare the results from our statistical data from section \ref{stat_data} with the results from ~\cite{Mockus2002a}, where we can se a similar pattern based on the schema we framed for overview data in section \ref{subsec:data}.



\subsection{Speculative}
%Peripheral developers participation F>C
Due to the differences that the we concluded in \ref{subsection:discussion_conclusive} for research question one and two, one could speculate what sort of implications there are in these differences. From this stand point there are implications that communities could have a higher peripheral developer participation which could result in (1) increasing product customer value, more eyes on the product would increase quality and (2) increasing the actual development speed. However from a firms perspective the participation from peripheral developers is already high and and implication are therefore by allowing peripheral developer to participate more could result in (1) lower development costs and (2) as mentioned before increased product customer value, more eyes on the product would increase quality. This also leads us into the speculation of the involvement by core developers, where we could se a low participation among firm driven projects and a high participation among community driven projects in previous section \ref{subsection:discussion_conclusive}. The reason for this particular phenonomen could be due to the simple reason that core developers in a firm driven OSS project take a supervising role to make sure that the work gets done instead of doing it them selfs. This could therefore be another implication for a community driven project to involve more peripheral developers into the process so that they them self can handel more critical problems within the development process.

Despite the similarities towards ~\cite{Noll} and ~\cite{Mockus2002a} articles we found an anomaly in our results data and in the research area who clearly confirms that the core developers of a open source project acknowledge the implementation of a feature into the software. The data we have collected shows that there are several occasions where peripheral developers acknowledge features into the software in both the Atom project and the Neovim project. The data can't provide the reader with the information to investigate why peripheral developers are allowed to acknowledge features in the projects, perhaps they have some sort of approval from a core developer to do this sort of judgment in the project, it could also be that there were core developers that don't belong to the organisation.

\subsection{Unforcen complications}
% discuss Barcelona, peripheral developer acknowledge, maybe they are not peripheral?
When analysing the result we found that the definition we used to categorise core and peripheral developers might not be accurate. We notice this due to the fact that there where a high amount of peripheral developers that made acknowledgments in both of the projects, a task that according to earlier research should be in the hands of a core developer. Furthermore we investigated this unforeseen complication and found a resent study written by~\cite{Krishnamurthy2016} that brings forward a more in depth description in how developers should be defined and categorised.
