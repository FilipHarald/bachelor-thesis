...
The overview data for Atom, Neovim and the other popular OSS projects was collected over a time period of a year, 2016-01-01 until 2017-01-01.
...
A first implementation of a feature is when a developer submits a piece of code which is intended to be an implementation of a feature. This is preceded with both a feature proposal and feature acknowledgement.
...
The first step involves several depending factors such as when a suggested implementation for a feature were made and where the feature origin came from based on three categories were the first category are if a proposal were asserted by a code contributor, either from his or her personal experience or knowledge of user needs, the second category are if the feature were proposed by an end-user, for example by posting a request to one of the discussion forums, and the last category focus if the feature were find while filing a bug report or request for enhancement in the issue database or derived from features found in competing products. The investigation of the first step is to us interesting due to the fact that you can se how high the contribution from non core developers are and therefore you can see if there are a general interest in proposing features and how that interest are handeld from the central group of core developers. 
