...
The first step involves several depending factors such as when a suggested implementation for a feature were made and where the feature origin came from based on three categories were the first category are if a proposal were asserted by a developer, either from his or her personal experience or knowledge of user needs, the second category are if the feature were proposed by an end-user, for example by posting a request to one of the discussion forums, and the last category focus if the feature were find while filing a bug report or request for enhancement in the issue database or derived from features found in competing products. The investigation of the first step is to us interesting due to the fact that you can se how high the contribution from non core developers are and therefore you can see if there are a general interest in proposing features and how that interest are handeld from the central group of core developers. 

The second step has no depending factor more than the first step because to acknowledge a feature there must be a feature proposal to acknowledge. So the second step is straightforward to investigate due to the fact that a proposal can either be acknowledged or not. We found this step interesting to investigate because we wanted to se if there were more or less feature acknowledgment in a firm-driven OSS project due to the high pressure on delivering a quality product that are associated with a company reputation.
...
The results and conclusion from our thesis opens up for opportunities to further research the subject. In the the following paragraphs we present suggestions for future work.
