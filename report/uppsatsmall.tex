...
Denna sammanfattning kommer skrivas inför slutgiltiga inlämningen.
...
In the second highly relevant article,~\cite{Noll}, Noll et al. presents a case study of the OpenEMR, an open source project developing electronic medical record (EMR) software. The goal of the study was to understand how requirements are elicited, documented, agreed and validated in a small open source software project. The study followed the same approach as of an earlier study of the Firefox web browser~\cite{Noll2008a}. The comparison between the both projects showed that, similar to the Firefox study, the majority of OpenEMR requirements are asserted by developers. Documentation was informal, consisting mainly of archived discussions. Contributors to the OpenEMR project were medical practitioners such as doctors or clinic administrations, who use the product in their practices and also developers which do not use the product at all. The implication for software development, in general, is that developers can be a significant source of innovation. In order to conduct the study the authors present a method including 5 steps for retrieving the data upon which they will make the comparison. The method is presented in Table~\ref{tab:openemr}.
...
\begin{table}
    \resizebox{!}{.35\paperheight}{\begin{tabular}{ p{0.6\textwidth} | p{0.15\textwidth} | p{0.1\textwidth} | p{0.4\textwidth}}
        ...
        Challenges and recommendations for the design and conduct of global software engineering courses: A systematic review    &  Snowballed    & 0 & - \\\hline
        ...
        Evolution in open source software: a case study    & \texttt{ACM} or \texttt{IEEE}    & 0 & Evolution in open source software AND case \\\hline
        ...
        Requirements elicitation in open source software development: a case study    & \texttt{ACM} or \texttt{IEEE}    & 3 & open AND source AND software AND development \\\hline
        ...
        The agile manifesto    & \texttt{GOOGLE}    & 0 & The + agile + manifesto \\\hline
...
\FloatBarrier
Based on the data presented in table~\ref{tab:github_stats}, we see that the (1) projects are similar in relation to a number of contributors, (2) both projects have several thousands of commits, and (3) both projects consists of several hundred of files.
...
The last, is the difference in the amount of LoC and files. As mentioned in section~\ref{neo_vim} Neovim is a product that uses a lot of the functionality from Vim. The result of this is that Neovim has a lot of code which has been transferred from Vim. Vim has 328722 LoC and 230 files. This justifies the difference in the LoC.
...
\newpage
\section{Results}
In the previous section~\ref{sec:method}, we presented our research methodology. In this section the results from the methods specified in section~\ref{sec:method} are presented. They are presented in the same order as they occur in section~\ref{subsec:data}. Which is first the overview data and then the detailed data.
...
The results of feature proposals displayed in Tables~\ref{tab:proposals_atom} and~\ref{tab:proposals_neovim} shows the amount of proposals made by individual users categorised as core developers, peripheral developers and end users. From this data we can summarise that there was a total of 20 feature proposals in the Atom project and 26 proposals in the Neovim project. Proposals from core developers in the Atom project adds up to a total of 4 and in the Neovim project they are a total of 12. Proposals made from contribution developers were in the Atom project a total of 12 in contrast to the Neovim project which had 11. Regarding proposals from end users the Atom project had 4 and the Neovim project had a total of 3.

In Table~\ref{tab:proposals_group} we can also see the number of unique users that that proposed a feature in each category in each project.The total amount of unique users among core developers ( core dev ) were 2 in the Atom project and 7 in the Neovim project. The biggest spread of unique users proposing features in both projects are from the category peripheral developers (dev) where Atom had a total of 9 unique peripheral developers and Neovim had a total of 10. In the category of end users all feature proposals could be mapped to unique users came from proposals from end users did not were only unique events where Atom had 4 end users proposing features and Neovim had a total of 3.
...
In this section we will analyse our results presented in the previous section. The analysis consists of three parts: validate the selection of cases and identify similarities and differences in requirement elicitation process.
...
%Section intro
In section \ref{sec:intro} we have present the problem, purpose, contribution and approach for this thesis. Furthermore in section \ref{subsec:background} we introduce the research area and describe the related work within the area. Based on these facts and thoughts a total of three research question were framed and presented in section \ref{sec:rq}. 

In this section we will discuss our findings in the previous section, their implications and their relevance to our first two research questions. Then we will discuss, based on the first two research questions, the answer to our third research question. We consider the answers to the first two questions to be conclusive, and the answer to our third research question to be speculative.
...
Communities and firms have approximately the same end-user involvement which means that when firms consider choosing OSS-development they can look at any OSS-project to see how end-users are involved.
...
When analysing the result we found that the definition we used to categorise core and peripheral developers might not be accurate, as mention in the previous section~\ref{sub:speculative}. We notice this due to the fact that there where a high amount of peripheral developers that made acknowledgments in both of the projects, a task that according to earlier research should be in the hands of a core developer. Furthermore, we investigated this unforeseen complication and found a recent study written by~\cite{Krishnamurthy2016} that brings forward a more in-depth description in how developers should be defined and categorised.

\newpage
\section{Conclusion and Suggestions for Future Work}
In this section we will summarise the answer for all three research questions. Then we will make suggestions for future work.
...
\begin{enumerate}[label=RQ\arabic*]
    ...
    \begin{itemize}
        ...
        \item Both firms and communities could, by encouraging contribution developers in an OSS-development with goals and opportunities, increase engagement among involved developers and improve the reputation towards developers not jet involved.
    \end{itemize}    
\end{enumerate}

In section~\ref{sec:unforeseen}  we mentioned that the definitions used for categorising developers into peripheral and core might not be accurate. Therefore it would be appropriate to conduct a similar study to ours but instead using the definitions of developer categories which, of course,  in~\cite{Krishnamurthy2016}.
