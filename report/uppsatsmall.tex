...
\begin{table}
	\resizebox{!}{.35\paperheight}{\begin{tabular}{  p{0.6\textwidth}  | p{0.15\textwidth} | p{0.1\textwidth} | p{0.4\textwidth}}
		...
		Evolution in open source software: a case study	& \texttt{ACM}	& 0 & ?? \\\hline
...
From the differences that the we concluded in~\ref{subsection:discussion_conclusive} for research questions one and two, one could speculate in what they implicate. Looking at peripheral developer participation the results implicate that communities could have a higher peripheral developer participation. This could result in (1) increasing product customer value, more eyes on the product would increase quality, and (2) increasing the actual development speed. However from a firms perspective the participation from peripheral developers is already high this could means that firms choosing to use an OSS development approach could (1) lower development costs (peripheral developers does not have a salary) and (2) as mentioned before: increase product customer value by having more eyes on the product. This also leads us into the speculation of the involvement by core developers, where we could see a low participation among firm driven projects and a high participation among community driven projects in previous section~\ref{subsection:discussion_conclusive}. The reason for this particular phenomenon could simply be due to that core developers in a firm driven OSS project take a supervising role to make sure that the work gets done instead of doing it by themselves. If this is the case a community driven project can also by involving more peripheral developers into the process the core developers can handle more critical problems within the development process by taking a supervising role.

Despite the similarities towards ~\cite{Noll} and ~\cite{Mockus2002a} articles we found an anomaly in our results and in the research area which clearly confirms that the core developers of an open source project acknowledge the implementation of a feature into the software. The data we have collected shows that there are several occasions where peripheral developers acknowledge features into the software in both the Atom project and the Neovim project. The data can't provide the reader with the information to investigate why peripheral developers are allowed to acknowledge features in the projects, perhaps they have some sort of approval from a core developer to do this sort of judgment in the project, it could also be that there were core developers that don't belong to the organisation.

\subsection{Unforseen complications}
