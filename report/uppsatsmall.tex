...
% section intro
In this section we will discuss our findings in previous the section, their implications and their relevance to our first two research questions. Then we will discuss, based on the first two research questions, the answer for our third research question. We consider the answers to the first two questions to be conclusive, and the answer for our third research question to be speculative.
...
TODO: When comparing to other study: Mozilla and OpenEMR has end-users that is not necessarily developers.

\subsection{Speculative}
%Peripheral developers participation F>C
Due to the differences that the we concluded in previous one could speculate what sort of implications there are in these differences. From this stand point there are implications that communities could have a higher peripheral developer participation which could result in (1) increasing product customer value, more eyes on the product would increase quality and (2) increasing the actual development speed. However from a firms perspective the participation from peripheral is already high and and implication are therefore by allowing peripheral developer to participate more could result in (1) lower development costs and (2) as mentioned before increased product customer value, more eyes on the product would increase quality. This also leads us into the speculation of the involvement by core developers, where we could se a low participation among firm driven projects and a high participation among community driven projects in previous section \ref{subsection:discussion_conclusive}. The reason for this particular phenonomen could be due to the simple reason that core developers in a firm driven OSS project take a supervising role to make sure that the work gets done instead of doing it them selfs. This could therefore be another implication for a community driven project to involve more peripheral developers into the process so that they them self can handel more critical problems within the development process.

\subsection{Unforcen complications}
% discuss Barcelona, peripheral developer acknowledge, maybe they are not peripheral?
When analysing the result we found that the defention we used to define core and peripheral developers might not be accurate. We notice this due to the fact that there where a high amount of peripheral developers that made acknowledgments in both of the projects, a task that according to earlier research should be in the hands of a core developer. Furthermore we investigated this unforseen complication and found a resent study wiritten by~\cite{Krishnamurthy2016}.
